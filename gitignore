...
def user_stats(df):
    ...
    #The following try clause is implemented to display the number of users by Gender
    #However, not every df may have the Gender column, so we have the following:
    try:
    ...
    #The earliest birth year and most recent birth year and the most common 
    #Here the birth years are displayed
    try:
...
#Main function to call all the functions from before
def main():
    while True:
